...
const createEscrow = asyncHandler(async (req, res) => {
    ...
    if (!project.freelancer) {
        throw new ApiError(400, "Project must have an assigned freelancer to create an escrow");
    }
...
const getEscrowByProject = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, escrow, "Escrow retrieved successfully")
    );
});
...
const fundEscrow = asyncHandler(async (req, res) => {
    ...
    if (milestoneIndex !== undefined) {
        ...
        if (amount !== milestone.amount) {
            throw new ApiError(400, `Amount must match milestone amount: ${milestone.amount}`);
        }
        ...
        escrow.transactions.push({
            ...
            amount,
            date: new Date(),
            reference: reference || `Milestone ${milestoneIndex + 1}: ${milestone.title}`,
            status: 'completed'
        });

        // Check if all milestones are now funded
        const allFunded = escrow.milestones.every(m => m.status === 'funded' || m.status === 'released');
        if (allFunded) {
        ...
        if (amount !== escrow.totalAmount) {
            throw new ApiError(400, `Amount must match total escrow amount: ${escrow.totalAmount}`);
        }
    ...
    return res.status(200).json(
        new ApiResponse(200, escrow, "Escrow funded successfully")
    );
});
...
const releaseFunds = asyncHandler(async (req, res) => {
    ...
    if (milestone.status !== 'funded') {
        throw new ApiError(400, "Cannot release funds for a milestone that is not funded");
    }
    ...
    escrow.transactions.push({
        ...
        amount: milestone.amount,
        date: new Date(),
        reference: reference || `Released milestone ${milestoneIndex + 1}: ${milestone.title}`,
        status: 'completed'
    });
    ...
    return res.status(200).json(
        new ApiResponse(200, escrow, "Funds released successfully")
    );
});
...
const initiateDispute = asyncHandler(async (req, res) => {
    ...
    if (
        ...
        throw new ApiError(403, "Only the client or freelancer can dispute an escrow");
    }
    ...
    return res.status(200).json(
        new ApiResponse(200, escrow, "Dispute initiated successfully")
    );
});
...
const resolveDispute = asyncHandler(async (req, res) => {
    ...
    if (!resolution || !['client-favor', 'freelancer-favor', 'settled'].includes(resolution)) {
        throw new ApiError(400, "Valid resolution is required (client-favor, freelancer-favor, or settled)");
    }
    ...
    if (resolution === 'client-favor') {
        ...
        const pendingMilestones = escrow.milestones.filter(m => m.status !== 'released');

        pendingMilestones.forEach((milestone, index) => {
            const actualIndex = escrow.milestones.findIndex(m => m._id.toString() === milestone._id.toString());
    ...
    return res.status(200).json(
        new ApiResponse(200, escrow, "Dispute resolved successfully")
    );
});
